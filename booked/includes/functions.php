...
function booked_appointments_available( $year = false, $month = false, $day = false, $calendar_id = false, $return_array = false, $include_past = false ){
	...
	if ( !$include_past && $buffer ):
		$buffered_timestamp = strtotime( date( 'Y-m-d H:i:s' ) . $buffer_string );//suncode
	else:
		$buffered_timestamp = false;
	endif;

	if ( !$include_past && $prevent_before ):
		$prevent_before = date('Ymd',strtotime($prevent_before));//suncode
	endif;

	if ( !$include_past && $prevent_after ):
		$prevent_after = date('Ymd',strtotime($prevent_after));//suncode
	endif;

	if ( !$day ):

		$month = date('m',strtotime($year.'-'.$month));//suncode
		$local_time = current_time('timestamp');
		$current_month = date('Ym',$local_time);//suncode

		if (!$include_past && $year.$month < $current_month):
			return 0;
		elseif (!$include_past && $year.$month > $current_month):
			$check_timestamp = strtotime(date('Y-m-d H:i:s',$local_time).' '.$buffer_string);//suncode
			...
			$start_timestamp = strtotime(date('Y-m-d H:i:s',$start_timestamp).' +'.floor($hours_between).' hours');//suncode
			$first_day = date('j',$start_timestamp);
			$last_day = date('t',strtotime($year.'-'.$month.'-01'));
			$end_timestamp = strtotime($year.'-'.$month.'-'.$last_day.' 23:59:59');
		elseif(!$include_past):
			$start_timestamp = strtotime($year.'-'.$month.'-'.date('d H:i:s',$local_time).' '.$buffer_string);
			$first_day = date('j',$start_timestamp);
			$last_day = date('t',strtotime($year.'-'.$month.'-01'));
			$end_timestamp = strtotime($year.'-'.$month.'-'.$last_day.' 23:59:59');
		else:
		...
		$local_time = current_time('timestamp');
		$current_day = date('Ymd',$local_time);

		if (!$include_past && $year.$month.$day < $current_day):
			return 0;
		elseif (!$include_past && $year.$month.$day > $current_day):
			$check_timestamp = strtotime(date('Y-m-d H:i:s',$local_time).' '.$buffer_string);
			...
			$start_timestamp = strtotime(date('Y-m-d',$start_timestamp).' +'.$hours_between.' hours');
			$first_day = date('j',$start_timestamp);
			$last_day = date('j',$start_timestamp);
	...
	$args = array(
		...
		'meta_query' => array(
			array(
				...
				'value'   => array( strtotime( date('Y-m-d',$start_timestamp).' 00:00:00'), $end_timestamp ),
				'compare' => 'BETWEEN',
			)
		)
	);
	...
	for($i = $first_day; $i <= $last_day; $i++){
		...
		$date_string = date('Ymd',strtotime($year.'-'.$month.'-'.$i));

		if (isset($booked_defaults[$date_string]) && empty($booked_defaults[$date_string])):
			continue;
		endif;

		$this_date_compare = date('Ymd',strtotime($year.'-'.$month.'-'.$i));
		...
		if (isset($booked_defaults[$date_string]) && !empty($booked_defaults[$date_string])):
			...
			foreach($booked_defaults[$date_string] as $timeslot => $count):
				...
				$date_check = date('Y-m-d',strtotime($date_string));
...
function booked_fe_calendar($year = false,$month = false,$calendar_id = false,$force_calendar = false){
	...
	$year = ($year ? $year : date('Y',$local_time));//suncode date_i18n('Y',$local_time));
	$month = ($month ? $month : date('m',$local_time));//suncode
	$today = date('j',$local_time); //suncode
	$currentMonth = date('Y-m-01',$local_time);//suncode date_i18n('Y-m-01',$local_time);
	...
	if (!$force_calendar):
		...
		$currentMonth = date('Y-m-01',strtotime($year.'-'.$month.'-01'));//suncode date_i18n('Y-m-01',strtotime($year.'-'.$month.'-01'));
	...
	?><div class="booked-calendar<?php echo ($booked_pa_active ? ' booked-pa-active' : ''); ?>"<?php echo ($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : ''); ?><?php echo (!$force_calendar ? ' data-calendar-date="'.$currentMonth.'"' : ''); ?>>
		<div class="bc-head">
			...
			$next_month = date('Y-m-01', strtotime("+1 month", strtotime($year.'-'.$month.'-01')));//suncode
			$prev_month = date('Y-m-01', strtotime("-1 month", strtotime($year.'-'.$month.'-01')));//suncode

			$next_month_compare = date('Ymd',strtotime($next_month));//suncode
		...
		<div class="bc-body"><?php
			...
			$today_date = date('Y',$local_time).'-'.date('m',$local_time).'-'.date('j',$local_time);//suncode
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					if ($buffer):
						...
						$currentTime = date('H:i:s',$buffered_timestamp);//suncode
					else:
						$date_to_compare = $local_time;
						$currentTime = date('H:i:s'); //suncode
					endif;

					$formatted_date = date('Ymd',strtotime($year.'-'.$check_month.'-'.$date)); //suncode
					...
					$this_date_compare = date('Ymd',strtotime($year.'-'.$check_month.'-'.$date));//suncode
...
//echo strtotime($year.'-'.$check_month.'-'.$date.' '.$currentTime)."--".$date_to_compare;
					if (
					...
					$check_month = date('m',strtotime($year.'-'.$check_month.'-'.$date));//suncode
					...
					$day_of_week = date('N',strtotime($check_year.'-'.$check_month.'-'.$date));//suncode
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		$year = date('Y',strtotime($date));//suncode
		...
		$date_display = date_i18n($date_format,strtotime($date));//must not changed by suncode
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($timeslot_parts[0] == '0000' && $timeslot_parts[1] == '2400'):
						...
						$timeslotText = date($time_format,strtotime($timeslot_parts[0])) . (!get_option('booked_hide_end_times') ? ' &ndash; '.date($time_format,strtotime($timeslot_parts[1])) : '');
					endif;
