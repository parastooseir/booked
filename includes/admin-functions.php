...
function booked_admin_calendar($year = false,$month = false,$calendar_id = false,$size = false){
	...
	$year = ($year ? $year : date('Y',$local_time));//suncode
		...
		<thead>
			<tr>
				<th colspan="<?php if (!$hide_weekends): ?>7<?php else: ?>5<?php endif; ?>">
					<a href="#" data-goto="<?php echo date('Y-m-01', strtotime("-1 month", strtotime($year.'-'.$month.'-01'))); ?>" class="page-left"><i class="fa-solid fa-angle-left"></i></a>
					...
					<span class="monthName">
						...
						<?php endif; ?>
						<?php echo date_i18n("F Y", strtotime($year.'-'.$month.'-01'));//must not change by suncode ?>
						<?php if ($calendar_name): ?>
							<span class="calendarName"><i class="fa-solid fa-calendar-days"></i>&nbsp;&nbsp;&nbsp;<?php echo $calendar_name; ?></span>
						<?php endif; ?>
					</span>
					<a href="#" data-goto="<?php echo date('Y-m-01', strtotime("+1 month", strtotime($year.'-'.$month.'-01'))); ?>" class="page-right"><i class="fa-solid fa-angle-right"></i></a>
				</th>
			</tr>
			<tr class="days">
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						$day_count++;
$persian_date = date_i18n('d M',strtotime($check_year.'-'.$check_month.'-'.$date));//suncode
						echo '<td data-date="'.$check_year.'-'.$check_month.'-'.$date.'" class="'.implode(' ',$classes).'">';
							echo '<span class="date'.($appointments_count > 0 ? ' tooltip' : '').'" '.($appointments_count > 0 ? ' title="'.sprintf(_n('%d Appointment','%d Appointments',$appointments_count,'booked'),$appointments_count).'"' : '').'><span class="number">'. $persian_date . '</span></span>';//suncode $date
						echo '</td>';
